...
const double temperature = 1000;   // kelvin
...
int main(int argc, const char **argv)
{
    ...
    for (int move=1; move<=num_moves; move = move + 1)
    {
        ...
        double V_old = box_size[0] * box_size[1] * box_size[2];

        const double old_box_size[3] = { box_size[0], box_size[1], box_size[2] };

        // Translate move - Pick a random atom
        ...
        if (rand(0.0 , 1.0) <= (1.0 / n_atoms)) {
            ...
            //Finally update the size of the box dimensions
            box_size[0] = box_side;
            box_size[1] = box_side;
            box_size[2] = box_side;
        ...
        else
        {
            ...
            const double x = exp( -(new_energy - old_energy + pressure * (V_new - V_old)) / kT + (n_atoms * (log(V_new) - log(V_old)) ) );
        ...
        double acc_ratio = naccept*100/move;
